...
MYSQLD_PATH=$(which mysqld_safe)

echo "Checking MySQL daemon.."
[ -z $(pidof mysqld_safe) ] && $MYSQLD_PATH &
sleep 10
...
# stop running mysqld to allow supervisord to supervise
[ ! -z $(pidof mysqld_safe) ] && killall -15 mysqld_safe
sleep 5
